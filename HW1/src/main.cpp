...
main()
{
    ...
    auto            TX = std::make_shared<Link>();
    auto            RX = std::make_shared<Link>();
    auto            disk_and_usb_gnd = std::make_shared<Link>();
    auto            disk_and_usb_vdd = std::make_shared<Link>();
    

    CPS_SET_OBJECT_NAME(esp8266);
    CPS_SET_OBJECT_NAME(vl530x);
    CPS_SET_OBJECT_NAME(USBnode);
    ...
    CPS_SET_OBJECT_NAME_PTR(TX);
    CPS_SET_OBJECT_NAME_PTR(RX);
    CPS_SET_OBJECT_NAME_PTR(disk_and_usb_gnd);
    CPS_SET_OBJECT_NAME_PTR(disk_and_usb_vdd);

    esp8266.gpio().vdd1.attachLink(disk_and_usb_vdd);
    esp8266.gpio().gnd1.attachLink(disk_and_usb_gnd);
    ...
    HardDisk my_hard_disk(&USBnode); 

    USBnode.gpio().vdd.attachLink(disk_and_usb_vdd);
    USBnode.gpio().gnd.attachLink(disk_and_usb_gnd);  
    USBnode.gpio().TX.attachLink(TX);
    USBnode.gpio().RX.attachLink(RX);
    USBnode.gpio().RX.setCanRead(false); // USB CAN'T READ FROM THIS PIN! 

    esp8266.gpio().tx.attachLink(TX);
    esp8266.gpio().tx.setCanRead(false); // MICROCONTROLLER CAN'T READ FROM THIS PIN!
    esp8266.gpio().rx.attachLink(RX);
